# charon-mono — 特性支持评估报告（v6 baseline）

## 元数据

- **数据源**：`runs/run-1778560393-59119/`（2026-05-12 v6 final，合并 verus rerun + R7 5-tool rerun）
- **工具配置**：`tools/charon-mono/`
- **工具版本**：`charon 0.1.184`（charon 自带 toolchain `nightly-2026-02-07-aarch64-apple-darwin`，与 `charon-poly` 共用同一可执行文件，仅 CLI flag `--monomorphize` 区别）
...
**项目维护的 wrapper**：无。直接调用上游 `charon` 二进制，不存在我方 shell 脚本。归因边界因此清晰——任何 driver panic 都是工具锅，按 §四.5 判定为 FAILED。
...
**0 误报（不冤枉能力）**：✅ 形式可证。
论证：charon `--abort-on-error` 路径下，exit ≠ 0 必经 `register_error!` 或显式 panic（`src/errors.rs:282`、`translate_trait_objects.rs:1707` 等）。源码层穷尽：所有 `register_error!` 调用点对应 unsupported / unimplemented 路径，无任何"假错"分支。exit 0 ⇔ 翻译完整无内部错误。
...
所有 8 个 FAILED 均 `exit_code=101`。逐条读 `runs/.../raw/charon-mono/*.stderr` 后归类。

### 桶 1：charon 翻译路径显式自陈 unsupported（4 case）

| entry | 自陈消息 |
|---|---|
| `charon-limit/async-fn/async_forty_two` | `Coroutine types are not supported yet`（`src/errors.rs:282`）|
| `kani-limit/async-await/run_async_add` | `Coroutine types are not supported yet`（同）|
| `charon-limit/inline-asm/nop_via_asm` | `Thread panicked when extracting body`（同上 panic 点，针对 `InlineAsm` MIR terminator）|
| `unsafe-ptr/raw-ptr-const/raw_ptr_const_match` | `Unsupported constant: ConstantExprKind::Cast {..}` |

**归因**：工具不支持。charon 在 MIR 提取阶段对 `Coroutine`、`InlineAsm`、`Cast` constant kind 显式 panic 自陈未实现。`--monomorphize` 不影响这些前端边界——与 charon-poly 表现一致（共享 FAILED）。
...
panic 源指向 `src/bin/charon-driver/translate/translate_trait_objects.rs:1707:13`。

**归因**：工具不支持（mono 模式独有边界）。`--monomorphize` 时对 trait object 的 vtable drop preshim 索引计算路径未实现；charon-poly 不展开 vtable 具体实例所以避开。`tools/charon-mono/README.md` 已记录此为 mono 模式已知触发点。

**处理**：不修。这是 charon mono 路径的实际能力边界，FAILED 站得住。
...
- **仍存在的盲点**：无已知。本次 161 entries 实测中未观察到 charon SUCCESS 但产物缺失 / 内部 skip 的案例
...
对称差合计 5 项，两 mode 在 FAILED 集合上不互含。

## 修订建议清单（仅"我们导致"失败）

**无需修订**。所有 8 个 FAILED 均归属工具能力边界（charon 自陈 unsupported / mono 路径 vtable preshim 未实现 / 内部 stack overflow），无任何"我方 wrapper / corpus / 环境"类问题——本工具无项目维护 wrapper，corpus 未触发 vendored crate lint，环境完好（无 binary 丢失 / 文件锁等）。

按 §四.5 判据，所有 FAILED 站得住，工具开发者不能驳回。
